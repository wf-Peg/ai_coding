...
## 骨架与依赖隔离
- [ ] `lite/` 目录结构完整（`main.js`、`preload.js`、`editor-file-service.js`、`frontend/editor.html`、`frontend/js/`、`frontend/libs/`、`frontend/styles/`）。
- [ ] `lite/package.json` 存在且独立，不引用完整版 `package.json`。
- [ ] `npm install` 后业务运行时仅使用 iconv-lite、chardet；Electron/electron-builder 位于 devDependencies，不含 sharp、node-notifier、js-yaml、serve-static。
- [ ] `main.js` 创建 900×640 原生标题栏窗口，菜单栏默认隐藏（Alt 唤出）。
- [ ] `preload.js` 以 contextIsolation:true、nodeIntegration:false 暴露 `window.liteAPI`。
- [ ] `main.js` 可读写 `{userData}/lite-config.json`，仅持久化 `fullVersionPath`。
- [ ] 启动时创建 `{userData}/notes/` 默认工作区，复用文件树和现有保存链路。
- [ ] workspace IPC 只读返回默认目录，渲染进程不能获得任意路径写权限。
...
## 启动完整版
- [ ] 开发模式（`!app.isPackaged`）下自动探测上级目录完整版，通过 `process.execPath` 启动，不依赖 shell PATH。
...
## 本地笔记
- [ ] Lite 启动后默认工作区为 `{userData}/notes/`。
- [ ] 文件树可新建/打开 `.txt`、`.md` 笔记并进入编辑。
- [ ] 编辑笔记后复用现有 autosave 保存，不覆盖其他文件。
- [ ] 重启 Lite 后可通过文件树重新打开笔记，内容完整不丢失。
...
- [ ] `libs/` 仅包含页面实际加载的文件，ACE worker 路径可正确加载。

## 回归与质量
- [ ] 所有 .js 文件语法检查通过（`node -c`）。
- [ ] 桌面模式冒烟测试通过（启动→编辑→保存→默认工作区笔记→启动完整版→全局快捷键→托盘菜单）。
