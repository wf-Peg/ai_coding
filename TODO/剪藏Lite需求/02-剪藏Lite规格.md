...
Lite 版的目标不是替代完整版，而是提供低成本的日常文本编辑入口，完整版作为按需启动的补充能力。通过代码复制而非共享依赖的方式，降低开发 token 消耗，同时保证 Lite 版构建独立性。

## What Changes

- 新建 `lite/` 子目录，包含完全独立的 `package.json`、Electron 主进程、preload 和前端文件。
- 复制现有编辑器模块核心文件（editor-core.js、editor-file-service.js、editor.css、design-tokens.css、主题文件、libs/ 目录、logger.js），保持原样零改动。
...
- 新增 `note-service.js`，提供本地笔记文件的读写能力，笔记存储在 `{userData}/notes/` 目录。
- 新增 `config-service.js`，提供 Lite 版配置读写（含完整版路径持久化）。
- 新增"启动完整版"功能，通过自动探测（开发模式 + 打包模式）和手动兜底选择实现跨实例启动。
- 新增全局快捷键 `Alt+X`（显示/隐藏窗口）和编辑器内快捷键 `Ctrl+Shift+O`（启动完整版）。
- 新增本地笔记功能：工具栏"笔记"按钮、笔记列表面板、新建笔记、自动保存。
...
- **Dependencies**: `lite/package.json` 独立管理依赖，仅 `electron` + `iconv-lite` + `chardet`，与完整版 `package.json` 无关联。
...
- **THEN** 仅安装 electron、iconv-lite、chardet 及其传递依赖，不安装 sharp、node-notifier、js-yaml、serve-static 等完整版专属依赖
...
- **THEN** 系统通过 `electron ..` 命令启动完整版，不弹窗询问
...
系统 SHALL 提供本地笔记功能，笔记以纯文本（.txt/.md）格式存储在 `{userData}/notes/` 目录，文件名使用时间戳格式；系统 SHALL 在工具栏提供"笔记"按钮打开笔记列表面板，支持新建笔记、列表浏览和点击载入编辑器。

#### Scenario: 新建笔记
- **WHEN** 用户点击"新笔记"按钮
- **THEN** 系统在 `{userData}/notes/` 下创建以当前时间戳命名的文件，立即进入编辑状态

#### Scenario: 笔记自动保存
- **WHEN** 用户编辑笔记内容
- **THEN** 系统复用现有 autosave 逻辑将内容保存到 notes/ 目录，不覆盖用户原始文件

#### Scenario: 笔记列表浏览
- **WHEN** 用户打开笔记列表面板
- **THEN** 系统展示 notes/ 目录下所有笔记的文件名和修改时间，点击列表项将笔记载入编辑器
