...
剪藏（Clip）是一款个人信息管理工具，帮助用户捕捉、整理和管理各类信息，构建个人知识库。

- **问题解决**：解决用户信息碎片化、难以有效管理和检索的问题，目标用户为需要频繁收集和整理信息的个人用户。
- **产品价值**：提高信息管理效率，通过AI辅助实现智能整理和分析，形成结构化的个人知识体系。
- **核心优势**：本地存储保障数据安全，AI智能分析提升信息价值，简洁直观的用户界面，支持多模型灵活切换。
...
8. **Git同步模块**：Git配置、同步仓库
9. **系统配置模块**：存储路径配置、邮箱配置、模型配置、Prompt配置、LLM路由配置
...
- **前端**：HTML5 + CSS3 + JavaScript，使用Marked.js渲染Markdown，支持Notion风格主题和常规主题切换
- **后端**：Spring Boot 3.2.0，Java 17，多LLM提供者抽象架构
- **AI服务**：多模型支持，包含DashScope（Qwen系列）和DeepSeek，通过LLM路由可灵活切换
- **存储**：本地文件系统，JSON格式存储剪藏内容，支持向量嵌入（Embedding）检索
...
- **桌面应用**：Electron 28+，electron-builder打包，支持Windows/macOS/Linux
...
- **LLM提供者抽象层**：`LlmProvider` 接口定义统一AI调用规范
- **DashScope提供者**：`DashScopeLlmProvider`，集成阿里云通义千问系列模型
- **DeepSeek提供者**：`DeepSeekLlmProvider`，集成DeepSeek大模型
- **路由提供者**：`RoutingLlmProvider`，按场景（分析、总结、标签生成、嵌入等）智能分配模型
- **模型配置**：支持自定义API地址、模型名称、API Key
...
│   │   ├── controller/               # 控制器（Clip、Todo、Topic、Git、ModelConfig、WeeklyReport）
│   │   ├── core/                     # 核心服务（AI、LLM提供者、路由、嵌入、定时任务）
│   │   ├── dto/                      # 数据传输对象
│   │   ├── model/                    # 数据模型（ClipContent、TodoContent、Topic、KnowledgeEntry）
│   │   ├── service/                  # 业务服务（Clip、Todo、Topic、搜索、邮件、Git、文档解析、链接解析等）
│   │   ├── utils/                    # 工具类（ImageUtils等）
...
│   ├── settings.js                   # 设置页面逻辑
│   ├── topic.js                      # 专题页面逻辑
│   ├── topic-detail.js               # 专题详情逻辑
│   ├── topic-editor.js               # 专题编辑器逻辑
│   ├── styles/                       # 样式文件
...
| 系统设置页面 | 配置表单   | 分组配置，模型配置、Prompt配置、存储配置、邮件配置         |
...
- **配置数据**：存储在用户目录`~/.clip-demo/`，包括Git配置、Prompt配置、模型配置等
...
- **桌面应用**：使用Electron打包为桌面应用（Windows .exe / macOS .dmg / Linux .AppImage），支持内嵌JRE
