...
6. 判断是否需「提炼进真相/决策」（Distill 环节，默认不做、避免负担）：
   - 方向 / 架构 / 状态级改动 → 同步 `HARNESS/roadmap.md` 的 Current/Done（保持「链接索引」形态，不复制内容）；
   - 被采纳的架构 / 约束取舍 → 写一条 ADR（`decisions/000N-*.md`，N = 现有最大编号 +1）。
   普通迭代只落 devlog 即可，不强制更新真相层。
```
