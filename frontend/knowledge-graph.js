(function() {
  ...
  // 知识图谱（只读语义关系图）：
  // 展示 剪藏 / 知识 / 学习计划 三类内容节点与 AI 算出的语义关系（derived_from / linked_to / plan_links）。
  // 定位为「只读视图」：节点位置由 force 自动布局决定，不提供手动画布卡片、手动连线、右键菜单、分组与 AI 生成。
  ...
  let autoLayoutMeta = null;              // 自动布局缓存 {nodeIds, svc}
  let panelDetailClipId = null;           // 剪藏节点详情「在编辑器中打开原文」携带的 clipId
  ...
  function buildGraph(nodes, links) {
    ...
    nodes.forEach(function(n) {
      if (n.canvas) return; // 只读语义图不展示手动画布卡片（便签/链接/图片/引用）
      var node = {
        id: n.id,
    ...
    links.forEach(function(link) {
      if (link.type === 'manual') return; // 手动连线不进语义图
    ...
    if (!allNodes.length) {
      ...
      emptyTitle.textContent = '暂无图谱数据';
      emptyDesc.textContent = '请先创建剪藏或知识条目并建立关联';
      return;
    }
  ...
  function initGraph() {
    ...
    var zoom = d3.zoom()
      .scaleExtent([0.05, 12])
      // 左键拖动留给力导向图自动布局的节点，不参与平移；滚轮缩放 / 空格或中键拖动平移
  ...
  function renderMinimap() {
    ...
    svgSel.append('g').selectAll('circle').data(allNodes).join('circle')
      ...
      .attr('r', function(d) { return Math.max(2, getNodeRadius(d) * minimapMeta.scale); });
  ...
  document.addEventListener('DOMContentLoaded', function() {
    // 进图即拉后端最新画布快照（仅影响本地 canvas 层，失败不阻塞语义图展示）
