 ...
 * 自研剪藏集成（mcp 桥 + clip-capture）已 profile 化，由 ensureCutshelterPlugins 写入
 * `~/.dsh/profiles/web/cordis.patch.yml`（手动 3080 与应用 3081 共享）。此处 --patch
 * 只保留运行时动态项（webserver 端口等），不再注入双插件，否则与 profile patch 同名
 * insert 重复注册导致 DSH 启动失败（duplicate loader entry id: mcp-cut-shelter）。
 */
function buildDshAgentPatch() {
  const config = loadConfig();
...
async function startDshAgent(config) {
  ...
  if (reused) {
    ...
    broadcastDshProgress('ready', `已复用现有 DSH 实例（端口 ${port}）`, port);
  ...
  try {
    fs.mkdirSync(CONFIG_DIR, { recursive: true });
    fs.writeFileSync(patchFile, buildDshAgentPatch(), 'utf-8');
  } catch (e) {
  ...
  while (Date.now() < deadline) {
    ...
    if (await checkHttpPort(primaryPort)) {
      ...
      return { success: true, reused: false, port: primaryPort };
    }
    const now = Date.now();
