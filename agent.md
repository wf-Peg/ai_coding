...
1. **探索交给子代理**：跨文件/大范围/模糊定位类探索（"哪里用到了 X"、"怎么接入 Y"）优先派 Explore 子代理，只取回结论摘要与关键文件:行号，**不全文读入主会话**。已知目标文件直接用 Read/Grep。
2. **读取瘦身**：
   - 大文件（如 `electron/main.js` 数千行）只读必要区段，用 `offset`/`limit` 精确切片，禁止整文件全文读取
   - 多个独立文件并行读取，减少来回轮次
3. **Grep 瘦身**：默认用 `files_with_matches` 或 `count` 模式；确需看内容时加 `head_limit`（≤ 60 行）并避免 `-A/-B/-C` 大上下文
4. **命令输出重定向**：`mvn`/`npm`/测试等长输出一律 `> /tmp/<任务>.log 2>&1`，只 `tail`/`grep` 关键行取结果，禁止全量日志进上下文
5. **避免重复执行**：同一验证命令一次跑完（如 `mvn test` 前确认输出有汇总段，不因截断重跑）；编译/测试结果以文件+关键行核验
6. **先想后动**：改动前先并行收集必要信息，避免"读一下-再读一下"的探索循环
...
3. **长回答必加小标题**：`### 大白话版` 与 `### 详细版` 两段式；无必要的短回答可省略。
