...
class FakeBrowserWindow {
  ...
  getPosition() { return this.pos || [0, 0]; }
...
const deps = {
  ...
  saveConfig: () => {},
  getMainWindow: () => ({ isDestroyed: () => false, minimize: () => {}, webContents: { send: () => {} } }),
  showMainWindow: () => {},
...
(async () => {
  ...
  console.log('== 2. F1 截图：即时反馈 → 抓屏回填(Buffer) ==');
  ...
  check('init 传的是 Buffer 非 base64', init && Buffer.isBuffer(init.payload.bg), typeof (init && init.payload.bg));
  check('init 带 mime=image/png', init && init.payload.mime === 'image/png');
