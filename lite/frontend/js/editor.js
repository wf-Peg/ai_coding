(function initializeLightEditor() {
  ...
  const elements = Object.fromEntries([
    'tabBar', 'tabNewBtn',
    'documentName', 'documentPath', 'modifiedDot', 'languageSelect',
    ...
    'encodingNote', 'discardModal', 'browserFileInput', 'toast',
    ...
    'launchFullBtn', 'liteAiSettingsBtn', 'liteAiSettingsModal', 'liteAiSettingsClose', 'liteAiProviderRadios', 'liteAiDeepseekKey', 'liteAiDeepseekModel', 'liteAiDashscopeKey', 'liteAiDashscopeModel', 'liteAiSaveBtn', 'liteAiShowKeyBtn'
  ...
  /**
   * Lite 版直接读取 preload 暴露的 liteAPI。
   */
  function getElectronAPI() {
    return window.liteAPI || null;
  }
  ...
  function updateDocumentIdentity() {
    ...
    elements.documentPath.textContent = state.displayPath || '尚未保存';
  ...
  async function sendAiMessage(rawMessage) {
    ...
    try {
      const aiConfig = await getLiteAiConfig();
      const apiMessages = window.EditorAiChatCore.toApiMessages(request.tab.aiChat);
      const result = await window.LiteAI.streamChat({
        config: aiConfig,
        messages: apiMessages,
        signal: request.controller.signal,
        onDelta: (delta) => {
          updateAiChatState(request.tab, {
            type: 'delta', assistantId: request.assistantId, content: delta
          });
        },
        onDone: () => {
        ...
        onError: (friendlyMsg) => {
          finishAiRequest(request, {
            type: 'error', assistantId: request.assistantId, message: friendlyMsg
          }, 'error');
        }
      });
      if (result && result.ok) {
        request.completed = true;
        if (!result.fellBack) {
          // onDone already handled
        } else {
          // 静默标记完成（实际由 onDone 直接处理）
        }
      }
    } catch (error) {
      if (request.controller.signal.aborted) return;
      if (request.completed) return;
      const friendlyMsg = window.LiteAI.friendlyMessage(error);
      finishAiRequest(request, {
        type: 'error', assistantId: request.assistantId, message: friendlyMsg
      }, 'error');
    } finally {
  ...
  function executeEditorContextAction(action) {
    ...
    if (action === 'smartIngest') {
      ...
      showToast('智能入库需要完整版，正在启动...');
      launchFullVersion();
      return;
    }
    if (action === 'aiImportPassword') {
      if (!selectedText.trim()) { showToast('请先选中文本', true); return; }
      showToast('AI 识别导入密码需要完整版，正在启动...');
      launchFullVersion();
      return;
    }
    mainEditor.focus();
  ...
  window.addEventListener('message', event => {
    ...
    if (data.action === 'themeChange' || data.type === 'themeChanged' || data.type === 'appearanceChanged') {
      ...
      try { window.parent.postMessage({ type: 'editorReady' }, '*'); } catch (_) {}
    } else if (data.action === 'openClipInEditor' || data.type === 'openClipInEditor') {
      showToast('查看剪藏需要完整版，正在启动...');
      launchFullVersion();
    } else if (data.action === 'refresh') {
      // Lite 版本无剪藏列表，refresh 触发同步为无害 no-op
  ...
  function triggerAutosave() {
    ...
    if (api.autosaveFile && state.fileToken) {
      liteAPI.file.save(state.fileToken, currentContent, state.encoding, state.lineEnding)
  ...
  function openFileTreeFile(file) {
    ...
    var api = getElectronAPI();
    if (api && liteAPI.file.openPath) {
      liteAPI.file.openPath(file.path)
        .then(function(result) {
  ...
  async function openRecentFile(filePath) {
    ...
    closeRecentPanel();
    try {
      const result = await liteAPI.file.openPath(filePath);
      if (!result || result.canceled) {
